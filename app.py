...
# Os cartões de KPI (com os ícones ⏱️⭐🔥✅) da Visão Geral aparecem AQUI,
# ACIMA do título e do botão "➕ Nova atividade" — isso evita qualquer corte
# visual que ocorria quando eles ficavam mais abaixo na página, disputando
# espaço/overflow com outros elementos.
if page == "🎯 Visão geral":
...
# ============================================================
# PÁGINA: VISÃO GERAL (KPI cards já foram renderizados acima, no cabeçalho)
# ============================================================
if page == "🎯 Visão geral":
...
# ============================================================
    ...
    for lvl in range(1, total_niveis_exibidos + 1):
        ...
        # Usa as classes .level-title/.level-status (definidas com !important
        # em theme.py) em vez de estilo inline — garante que o nome do nível
        # apareça com cor visível independentemente do tema (claro/escuro)
        # que o Streamlit aplicar por cima do conteúdo em Markdown.
        with lvl_cols[(lvl - 1) % 2]:
            st.markdown(
                f"""<div class="level-card {card_class}">
                        <div class="level-badge {badge_class}">{lvl}</div>
                        <div>
                            <div class="level-title">Nível {lvl}: {dm.level_name(lvl)}</div>
                            <div class="level-status">{status_txt}</div>
                        </div>
                    </div>""",
                unsafe_allow_html=True,
            )
...
# ============================================================
    ...
    if not df_atrasadas_config.empty:
        # Caixa customizada (fundo verde-oliva escuro / texto verde-claro),
        # em vez do st.warning padrão, para bater com a cor de destaque
        # escolhida para alertas de atraso em todo o app.
        st.markdown(
            f"<div class='overdue-alert'>Você tem {len(df_atrasadas_config)} tarefa(s) pendente(s) atrasada(s).</div>",
            unsafe_allow_html=True,
        )
